          ...
          <div className="space-y-8">
            <div className="bg-background/60 backdrop-blur-sm rounded-lg p-6 border">
              {isLoading ? renderProfileSkeleton() : <div className="flex flex-col md:flex-row gap-6 items-center md:items-start">
                  <Avatar className="w-32 h-32 border-4 border-background">
                    ...
                    <div className="flex-1 space-y-3 text-center md:text-left">
                    <div className="flex flex-wrap items-start justify-center md:justify-between gap-2">
                      <div className="flex items-start gap-1">
                        <h1 className="text-3xl font-bold text-icc-gold">{profile?.display_name}</h1>
                        {badgeInfo && <div className="-mt-1">
            ...
            <Tabs defaultValue="reports" className="w-full" value={activeTab} onValueChange={setActiveTab}>
              <TabsList className="w-full justify-start overflow-x-auto bg-background/60 backdrop-blur-sm rounded-lg border p-1 mb-6">
                <TabsTrigger value="reports" className="data-[state=active]:bg-icc-gold/20" style={{ color: '#C8B06B' }}>
                  {isMobile ? <FileText className="h-5 w-5" /> : <>
                      <FileText className="h-4 w-4 mr-2" />
                      Reports
                    </>}
                </TabsTrigger>
                <TabsTrigger value="bounties" className="data-[state=active]:bg-icc-gold/20" style={{ color: '#C8B06B' }}>
                  {isMobile ? <Package className="h-5 w-5" /> : <>
                      <Package className="h-4 w-4 mr-2" />
                      Bounties
                    </>}
                </TabsTrigger>
                <TabsTrigger value="wallet" className="data-[state=active]:bg-icc-gold/20" style={{ color: '#C8B06B' }}>
                  {isMobile ? <WalletIcon className="h-5 w-5" /> : <>
                      <WalletIcon className="h-4 w-4 mr-2" />
                      Wallet
                    </>}
                </TabsTrigger>
                <TabsTrigger value="info" className="data-[state=active]:bg-icc-gold/20" style={{ color: '#C8B06B' }}>
                  {isMobile ? <Info className="h-5 w-5" /> : <>
                      <Info className="h-4 w-4 mr-2" />
                      Info
                    </>}
                </TabsTrigger>
                <TabsTrigger value="activity" className="data-[state=active]:bg-icc-gold/20" style={{ color: '#C8B06B' }}>
                  {isMobile ? <ThumbsUp className="h-5 w-5" /> : <>
                      <ThumbsUp className="h-4 w-4 mr-2" />
                      Agreed
                    </>}
                </TabsTrigger>
                <TabsTrigger value="comments" className="data-[state=active]:bg-icc-gold/20" style={{ color: '#C8B06B' }}>
