      ...
      if (event === 'SIGNED_IN') {
        ...
        if (sessionData?.user?.email) {
          ...
          if (walletFromEmail && walletFromEmail !== 'null') {
            ...
            try {
              // Use a shorter delay since we have better loading state management now
      ...
      provider.on('connect', async () => {
        ...
        if (publicKey) {
          ...
          try {
            ...
            // Ensure loading state is cleared if auth state change handler hasn't done it
            setTimeout(() => {
              setIsLoading(false);
            }, 2000); // Give auth state change handler time to complete
          }
        }
      });
