...
const BadgeTier: React.FC<BadgeTierProps> = ({ 
  ...
  const { tier, icon, color, nextTier } = badgeInfo;
  ...
  const badge = (
    <Badge 
      className={`${sizeClasses[size]} ${color} border font-medium`}
    >
      <span className={`mr-1 ${iconSizes[size]}`}>{icon}</span> {tier}
    </Badge>
  );
          ...
          <TooltipContent className="p-4 max-w-sm">
            <div className="space-y-3">
              ...
              <div className="text-sm text-muted-foreground">
                ...
                {badgeInfo.percentOfSupply > 0 && (
                  <span> ({badgeInfo.percentOfSupply}% of total supply)</span>
                )}
              </div>
              ...
              {showProgress && nextTier && (
                <div className="space-y-2">
                  ...
                  <Progress 
                    value={100 - ((nextTier.remaining / (nextTier.minHolding - badgeInfo.minHolding)) * 100)} 
                    className="h-2" 
                  />
